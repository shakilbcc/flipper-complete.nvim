...
describe('cmp source', function()
  ...
  it('get_trigger_characters returns quote and colon chars', function()
    local cmp_source = require('flipper-complete.cmp')
    local source = cmp_source.new()
    assert.same({ ':', '"', "'" }, source:get_trigger_characters())
  end)

  it('is_available returns true when file exists', function()
    local cmp_source = require('flipper-complete.cmp')
    local source = cmp_source.new()
    assert.is_true(source:is_available())
  end)

  it('is_available returns false when file does not exist', function()
  ...
  it('complete calls callback with items on match', function()
    ...
    source:complete({
      context = {
        cursor_before_line = 'Features.enabled?("',
        cursor_after_line = '',
        cursor = { row = 4, col = 21 },
      },
    }, function(response)
